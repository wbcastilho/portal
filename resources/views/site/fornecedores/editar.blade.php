...
@section('subtitulo')
    Fornecedores <small>(Formulário)</small> 
@endsection
...
@section('content')   
    ...
    <form id="meuForm" class="form" role="form" action="{{ route('fornecedores.store') }}" enctype="multipart/form-data" method="post">
        @csrf
        @method('PUT')
        ...
        <div class="box-body">
            <div class="row">
                <div class="form-group col-md-2 col-sm-2 col-xs-2">                                                              
                    ...
                    <input disabled type="text" id="id" name="id" class="form-control" value="{{ isset($fornecedor->id) ? $fornecedor->id : '' }}{{old('id')}}">                                 
                </div> 
            </div>

            @include('site.fornecedores._form')                   
        </div>                       
    ...
    <input type="hidden" name="hidId" id="hidId" value="{{ isset($fornecedor->id) ? $fornecedor->id : '' }}{{old('id')}}" >
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Informações do fornecedor alteradas com sucesso!</p>
                </div>
         ...
         $('#btnSalvar').click(function (e) {
            ...
            axios({
                ...
                data: { 
                    ...
                    ie: $("input[type=text][name=ie]").val(),
                    cnpj: $("input[type=text][name=cnpj]").val(),
                    estado_id: $("select[name=estado_id]").val(),                                                        
                    cidade_id: $("select[name=cidade_id]").val(),
                    endereco: $("input[type=text][name=endereco]").val(),
                    numero: $("input[type=text][name=numero]").val(),
                    bairro: $("input[type=text][name=bairro]").val(),
                    telefone: $("input[type=text][name=telefone]").val(),
                    celular: $("input[type=text][name=celular]").val(),

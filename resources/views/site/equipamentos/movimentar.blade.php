...
@section('subtitulo')
    Equipamentos <small>(Movimentar)</small> 
@endsection
...
@section('content')   
    ...
    <form id="meuForm" class="form" role="form" action="{{ route('equipamentos.movimentacao',$equipamento->id) }}" enctype="multipart/form-data" method="post">
        @csrf
        ...
        <div class="box-body"> 
            <div class="row">
                <div class="form-group col-md-2 col-sm-2 col-xs-2">                                                              
                    ...
                    <input disabled type="text" id="id" name="id" class="form-control" value="{{ isset($equipamento->id) ? $equipamento->id : '' }}{{old('id')}}">                                 
                </div> 
            </div>
            ...
            <hr>  
            <div class="row">
                <div class="form-group col-md-4 col-sm-4 col-xs-12 {{ $errors->has('situacao_id') ? 'has-error' : ''}}">
    ...
    <input type="hidden" name="hidId" id="hidId" value="{{ isset($equipamento->id) ? $equipamento->id : '' }}{{old('id')}}" >
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Movimentação realizada com sucesso!</p>
                </div>
...
@section('js')
    ...
    <script>
        ...
        $('select[name=situacao_id]').change(function () {
            var situacao_id = $(this).val();

            if(situacao_id != 2){
                $("select[name=estado_id]").parent().parent().fadeOut();
                $("select[name=cidade_id]").parent().parent().fadeOut();
                $("select[name=localizacao1_id]").parent().parent().fadeOut();
                $("select[name=localizacao2_id]").parent().parent().fadeOut();
                $("select[name=localizacao3_id]").parent().parent().fadeOut();
                $("select[name=localizacao4_id]").parent().parent().fadeOut();               
            }
            else {
                $("select[name=estado_id]").parent().parent().fadeIn();
                $("select[name=cidade_id]").parent().parent().fadeIn();
                $("select[name=localizacao1_id]").parent().parent().fadeIn();
                $("select[name=localizacao2_id]").parent().parent().fadeIn();
                $("select[name=localizacao3_id]").parent().parent().fadeIn();
                $("select[name=localizacao4_id]").parent().parent().fadeIn();  				
            }
        });
         ...
         $('#btnSalvar').click(function (e) {
            ...
            axios({
                ...
                url: "{{ route('equipamentos.index') }}" + "/" + id + "/movimentar/movimentacao", // url                       
                data: {                      
